...
exports.addArticleCates = (req, res) => {
    ...
    db.query(sql2, [req.body.name, req.body.alias], (err, results) => {
        ...
        if (results.length === 2) { res.cc('分类名称与别名被占用，请更换后重试！'); return; }
        // 分别判断 分类名称 和 分类别名 是否被占用
        if (results.length === 1 && results[0].name === req.body.name) { res.cc('分类名称被占用，请更换后重试！'); return; }
        if (results.length === 1 && results[0].alias === req.body.alias) { return res.cc('分类别名被占用，请更换后重试！'); return; }
...
exports.updateCateById = (req, res) => {
    // 定义查询 分类名称 与 分类别名 是否被占用的 SQL 语句
    const sql5 = `select * from ev_article_cate where Id<>? and (name=? or alias=?)`
    // 执行查重操作
    db.query(sql5, [req.body.Id, req.body.name, req.body.alias], (err, results) => {
        // 执行 SQL 语句失败
        if (err) return res.cc(err)
        ...
        // TODO：更新文章分类
        const sql6 = `update ev_article_cate set ? where Id=?`
